...
static void http_rest_with_url(void)
{
    ...
    esp_http_client_config_t config = {
        ...
        .url = "https://api.seniverse.com/v3/weather/now.json?key=S-Eib0YnZZP2_iRJa&location=beijing&language=zh-Hans&unit=c",
    ...
    cJSON *monitor_json = cJSON_Parse(local_response_buffer);
    ...
    results = cJSON_GetObjectItem(monitor_json, "results");
    printf("test\r\n");
    //printf("Checking monitor \"%s\"\n", cJSON_GetStringValue(results));
    // if (cJSON_IsString(results) && (results->valuestring != NULL))
    // {
    //     printf("Checking monitor \"%s\"\n", results->valuestring);
    // }
    result_array = cJSON_GetArrayItem(results, 0);
    const cJSON *now_object = NULL;
    now_object = cJSON_GetObjectItem(result_array, "now");
    const cJSON *text = NULL;
    text = cJSON_GetObjectItem(now_object, "text");
    printf("the test is %s\r\n", cJSON_GetStringValue(text));

    // cJSON_ArrayForEach(result, results)
    // {
    //     //cJSON *location = 
    //     cJSON *now =cJSON_GetObjectItemCaseSensitive(results, "now");
...
void app_main(void)
{
    ...
    OLED_Init();
    OLED_Clear();
    //OLED_ShowString(30,0,(u8 *)"OLED TEST");
    OLED_ShowCHinese(0,0,7);//?D
